...
def panorama():
    ...
    tab1, tab2, tab5, tab6 = st.tabs(
        ["Índices", "Criptomoedas", "Yeld", "Commodities"])
    ...
    with tab1:
        ...
        for ticker in index_tck.keys():
            if not ticker == indice_selecionado:
                ...
                indice_diario = yf.download(index_tck.get(
        ...
        fig.update_layout(title=indice_selecionado,
    ...
    with tab2:
        ...
        for ticker in crypto_tck.keys():
            if not ticker == indice_selecionado:
                ...
                indice_diario = yf.download(crypto_tck.get(
                    indice_selecionado), period="60d", interval="1d")
...
def main():
    ...
    st.sidebar.markdown("---")
    lista_menu = ["Panorama", "Home", "Rentabilidade Mensal", "Fundamentos"]
